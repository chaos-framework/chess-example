...
export default class DiagonalMovementOnly extends Component implements Modifier {
  name = "Diagonal Movement Only"
  ...
  modify(action: Action) {
    if (action instanceof MoveAction && action.target === this.parent && action.tagged('PLAYER_MOVEMENT')) {
      ...
      if(!target.position.isDiagonalTo(to)) {
        action.deny({ message: 'Can only move diagonally.'});
      }
    }
  }
}