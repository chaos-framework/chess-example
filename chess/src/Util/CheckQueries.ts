...
import { ChessPiece } from "./Types.js";

export function movementWillResultInCheck(
  board: World,
  checkablePiece: ChessPiece,
...
export function isInCheck(board: World, piece: ChessPiece, depth = 0): boolean {
...
export function isInCheckmate(
  ...
  for (let x = 0; x < 8; x++) {
    for (let y = 0; y < 8; y++) {
      ...
      if (
        ...
        moveIsPossible(piece, position, depth + 1)
      ) {
        return false;
      }
    }
  ...
  const positionsBetweenAttackerAndPiece = [
    ...piece.position.getLineTo(attacker.position),
  ];
...
function getAlliedPieces(board: World, friendlyTeam: Team): ChessPiece[] {
  const enemyPieces: ChessPiece[] = [];
  for (const [, entity] of board.entities) {
    if (entity.team === friendlyTeam) {
      enemyPieces.push(entity as ChessPiece);
    }
  }
  return enemyPieces;
}

function getEnemyPieces(board: World, friendlyTeam: Team): ChessPiece[] {
  const enemyPieces: ChessPiece[] = [];
  for (const [, entity] of board.entities) {
    if (entity.team !== friendlyTeam) {
      enemyPieces.push(entity as ChessPiece);
