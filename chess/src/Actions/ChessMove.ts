...
export default class ChessMove extends MoveAction {
  ...
  constructor(public target: Entity, to: string | Vector, public queryDepth = 0, options: ActionParameters = {}) {
    super({ target, to: to instanceof Vector ? to : Chessboard.fromAlgebraic(to) || new Vector(0,0), ...options});
    this.deny();
  }
